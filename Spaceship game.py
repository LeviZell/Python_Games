# -*- coding: utf-8 -*-
"""
Created on Thu Nov 10 14:06:15 2016
...
class Ship:
    ...
    def shoot(self):
        ...
        missile_group.add(Sprite(missile_pos, missile_vel, 
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        # increase rock velocity w/ increase in score
        if score > 1:
            rock_vel = [random.random() * (.6 + score*.2)  - .3, random.random() * (.6+ score *.2) - .3]
        else:
            rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
        rock_avel = random.random() * .2 - .1
        # Determines if rock position will spawn too close to my_ship
        while dist(rock_pos, my_ship.pos) < 100:
            rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_group.add(Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info))
#    for r in rock_group:
#        rock_group = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
...
def group_group_collide(group, other_object):
    counter = 0
    for s in set(group):
        if group_collide(other_object, s):
#            gg_removed_set.add(s)
            group.remove(s)
            counter += 1
    return counter    
